  ...
  return (
    <div className="import-asset-item-section">
      ...
      <div className="import-asset-item-section-label">
        <EllipsisMiddle suffixCount={8}>{isHex ? item.asset_id : window.btoa(item.asset_id)}</EllipsisMiddle>
      </div>
    </div>
  );
}
  ...
  const assetList = useMemo(() => {
    ...
    if (list?.length) {
      return list.map((item) => {
        ...
        return (
          <div className="import-asset-item" key={item.asset_id}>
            <Tooltip
            ...
            >
              {!device.isMobile && <div className="import-asset-item-logo">
                <Image
                  width={40}
                  height={40}
              ...
              <div className="import-asset-item-name">
                <div className="import-asset-item-section-label color-dark">Asset Name</div>
                <div className="import-asset-item-section-value">
                  <EllipsisMiddle suffixCount={8} suffixCountMore={-6}>
                    {item.asset_name}
                  </EllipsisMiddle>
                </div>
              </div>
              <div className="import-asset-item-type">
                <div className="import-asset-item-section-label color-dark">Asset Type</div>
                <div className="import-asset-item-section-value">{assetsTypeMap[item?.asset_type] || "--"}</div>
              </div>
      ...
      return (
        <div className="import-asset-list-empty">
          <Empty
            image={Empty.PRESENTED_IMAGE_SIMPLE}
            imageStyle={{ color: "#fff" }}
            description={
              <span className="color-base f16">
                Can't find this asset, maybe this haven’t import to NostrAssets from the universe, please sync the
                universe and import.
              </span>
            }
          />
    ...
    <>
      <div className="import-asset-container">
        ...
        <div className="import-asset-content">
          <div className="import-asset-content-about">
            <span className="sync-link" onClick={() => dispatch(setAboutModalVisible(true))}>
              What is Import Assets?
            </span>
          </div>
          <div className="import-asset-title">Import Assets</div>
          ...
          <div className="import-asset-search">
            ...
            <div className="relative">
              <Input onChange={assetIdChange} size={device.isMobile ? "middle" : "large"} placeholder="Search by asset name or asset ID" />
          ...
          <div className="import-asset-list">{assetList}</div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button type="primary" size={"middle"} onClick={handleImportOk}>
              OK
            </Button>
          </>
        }
      ...
      >
        <div>
          <div className="tc">
            {importingMap.type == "success" ? (
              <CheckCircleOutlined style={{ fontSize: "30px", color: "#38c89d" }} />
            ) : (
              <CloseCircleOutlined style={{ fontSize: "30px", color: "#ff4400" }} />
            )}
          </div>
          <div style={{ marginTop: "20px" }}>{importingMap.content}</div>
        </div>
      </Modal>
    </>
  );
