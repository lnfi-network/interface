...
export default function ExploreDetails({ detail, open = false, onClose, type }) {
  ...
  const qutoAssetDetail = useMemo(() => {
    return tokenList.find((item) => item?.name?.toUpperCase() == QUOTE_ASSET);
  }, [tokenList]);
  ...
  const messageDetail = useMemo(() => {
    const data = detail?.message_detail ? JSON.parse(detail?.message_detail) : [];
        ...
        <>
          <div className="explore-detail-list-item">
            ...
            <span className="explore-detail-list-item__text">
              {detail?.messageid ? <EllipsisMiddle suffixCount={6}>{detail?.messageid}</EllipsisMiddle> : "--"}
            </span>
          </div>
          <div className="explore-detail-list-item">
            ...
            <span className="explore-detail-list-item__text">
              {detail?.plaintext_context ? (
                <EllipsisMiddle suffixCount={8}>{detail?.plaintext_context}</EllipsisMiddle>
              ) : (
                "--"
              )}
            </span>
          ...
          <div className="explore-detail-list-item">
            ...
            <span className="explore-detail-list-item__text">{detail?.type || "--"}</span>
          </div>
          <div className="explore-detail-list-item">
            <span className="explore-detail-list-item__label">{t`Address`}</span>
            <span className="explore-detail-list-item__text">
              {detail?.nostr_address && (
                <EllipsisMiddle suffixCount={6}>{detail?.nostr_address || "--"}</EllipsisMiddle>
              )}
            </span>
          </div>
          <div className="explore-detail-list-item">
            ...
            return (
              <div key={item.OrderID + i}>
                ...
                <div className="explore-detail-list-item">
                  ...
                  <span className="explore-detail-list-item__text">{item?.OrderID || "--"}</span>
                </div>
                <div className="explore-detail-list-item">
                  <span className="explore-detail-list-item__label">{t`Type`}</span>
                  <span
                ...
                <div className="explore-detail-list-item">
                  ...
                  <span className="explore-detail-list-item__text">
                    {curToken ? (
                      item?.Amount ? (
                        numberWithCommas(
                          limitDecimals(BigNumber(item?.Amount).div(curToken?.decimals).toNumber()),
                          curToken?.reserve
                        )
                      ) : (
                        "--"
                  ...
                  <span className="explore-detail-list-item__text">
                    {qutoAssetDetail ? (
                      item?.Price ? (
                        numberWithCommas(
                          limitDecimals(
                            BigNumber(item?.Price).div(qutoAssetDetail?.decimals).toNumber(),
                            qutoAssetDetail?.reserve
                          )
                        )
                      ) : (
                        "--"
